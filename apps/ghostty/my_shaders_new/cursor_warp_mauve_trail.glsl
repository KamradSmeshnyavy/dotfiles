// Smooth cursor warp + mauve trail color from cursor.glsl
// Base motion/easing logic is taken from cursor_warp.glsl

// --- CONFIGURATION ---
const vec4 TRAIL_COLOR = vec4(0.7961, 0.6510, 0.9686, 1.0); // color from cursor.glsl
...
const float THRESHOLD_MIN_DISTANCE = 1.5; // min distance to show trail (cursor-height units)
const float BLUR = 1.0; // blur size in pixels (antialiasing)
const float TRAIL_THICKNESS = 1.0;
const float TRAIL_THICKNESS_X = 0.9;

const float FADE_ENABLED = 0.0; // 1.0 to enable fade gradient along the trail
const float FADE_EXPONENT = 5.0;

// --- CONSTANTS for easing functions ---
const float PI = 3.14159265359;

// EaseOutCirc (same smooth easing as cursor_warp.glsl)
float ease(float x) {
...
// Determines animation duration based on a corner's alignment with the move direction.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (lineLength > minDist && baseProgress < DURATION - 0.001) {
        // Current cursor corners with custom thickness
        ...
        // Previous cursor corners with custom thickness
        ...
        if (BLUR < 2.5) {
          // no antialiasing on horizontal/vertical movement to avoid pulse
        ...
        // Accent layer (same color set as cursor.glsl)
        newColor = mix(newColor, TRAIL_COLOR_ACCENT, 1.0 - smoothstep(sdfTrail, -0.01, 0.001));

        vec4 trail = TRAIL_COLOR;
        if (FADE_ENABLED > 0.5) {
            float easedFade = pow(fadeProgress, FADE_EXPONENT);
            trail.a *= easedFade;
        }
        ...
        // Punch hole on the trail so current cursor is always crisp on top.
