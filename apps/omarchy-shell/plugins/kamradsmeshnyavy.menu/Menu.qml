...
Item {
  ...
  // The menu draws itself with a skin instead of the stock card list. Kept as
  // a switch rather than a rewrite so the packaged look stays one
  // `omarchy plugin enable omarchy.menu` away if none of them stick.
  // dmenu prompts (theme pickers, text input) keep the card: a select list is
  // the right shape for them.
  property string skin: skinConfig.skin
  ...
  readonly property string skinTitle: root.activeMenu === "root"
    ? "Omarchy"
    : (root.item(root.activeMenu) ? (root.item(root.activeMenu).title || root.item(root.activeMenu).label) : "Go")
  readonly property string skinSubtitle: root.activeMenu === "root" ? "" : root.pathFor(root.activeMenu)
  ...
  // A cloned plugin gets a scoped shell whose appLibrary is null, so the app
  // list falls back to reading desktop entries directly.
  function appEntries() {
    if (!root.appLibrary) return appSource.entries()
  ...
  function appEntryName(entry) {
    return root.appLibrary ? root.appLibrary.entryName(entry) : String(entry.name || "")
  }

  function appEntrySubtext(entry) {
    return root.appLibrary ? root.appLibrary.entrySubtext(entry) : String(entry.genericName || entry.comment || "")
  }
  ...
  function appIconSource(icon) {
    ...
    var resolved = root.appLibrary ? root.appLibrary.iconSource(key) : appSource.iconFor(key)
  ...
  function launchApp(appId, label) {
    ...
    else appSource.launch(appId, label)
  }

  // The apps provider is QML-native: rows come from desktop entries instead of
  // a bash enumeration, so they carry image icons and launch support.
  ...
  // Entrance animations belong to the navigation that started them, not to
  // whatever asynchronous load lands afterwards.
  Timer {
    id: burstWindow
    interval: 520
    onTriggered: root.burst = false
  }

  AppSource {
    id: appSource
    onAppsChanged: if (root.providersLoaded["apps"]) root.mergeAppRows()
  }
  ...
  Process {
    ...
    onExited: function(exitCode, exitStatus) {
      ...
      // Rebuilding drops and recreates every row, so the skins replay their
      // entrance animation and the launcher looks like it blinked out and
      // came back. Guards almost always answer the same thing they answered
      // last open, so only pay for it when something actually moved.
      var changed = !MenuModel.sameGuardResults(root.whenResults, nextWhen)
        || !MenuModel.sameGuardResults(root.checkedResults, nextChecked)
