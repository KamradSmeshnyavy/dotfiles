...
def run_command(cmd, shell=False, cwd=None):
    try:
        ...
        result = subprocess.run(
            cmd, shell=shell, check=True, cwd=cwd or get_repo_root()
        )
...
def run_dotbot(config_file):
    ...
    if not dotbot_bin.exists():
        print(
            f"{COLOR_ERROR}[ERROR]{COLOR_RESET} dotbot executable not found: {
                dotbot_bin
            }"
        )
        return False
    ...
    if not config_path.is_absolute():
        config_path = repo_root / "install" / config_file
